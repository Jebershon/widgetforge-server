...
async function buildWidgetPackage(
  ...
  try {
    ...
    // 1. Fix widget ID in the root <widget> tag
    if (!finalXml.includes(`id="${expectedId}"`)) {
      finalXml = finalXml.replace(/(<widget\s+[^>]*?id=")[^"]*(")/i, `$1${expectedId}$2`);
    }

    // 2. Ensure all properties are inside <properties> and wrapped in <propertyGroup>
    // We only do this if the AI failed to provide a valid structure.
    if (!finalXml.includes('<properties>') || !finalXml.includes('<propertyGroup')) {
      console.log("  [xml-fix] Standard Mendix structure missing. Attempting restoration...");
      
      // Extract everything that looks like a property or propertyGroup
      const propertyRegex = /<(property|propertyGroup)[\s\S]*?<\/\1>/gi;
      const matches = finalXml.match(propertyRegex) || [];
      let propertyBlock = matches.join('\n');

      if (propertyBlock) {
        // If there are properties but no group, wrap them
        if (!propertyBlock.includes('<propertyGroup')) {
           propertyBlock = `        <propertyGroup caption="General">\n${propertyBlock}\n        </propertyGroup>`;
        }
        
        // Remove the old property-related tags from finalXml to avoid duplicates
        finalXml = finalXml.replace(propertyRegex, '');
        
        // Re-inject into <properties> block
        if (finalXml.includes('<properties>')) {
           finalXml = finalXml.replace(/<properties>[\s\S]*?<\/properties>/i, `<properties>\n${propertyBlock}\n</properties>`);
        } else {
           finalXml = finalXml.replace(/(<\/widget>)/i, `    <properties>\n${propertyBlock}\n    </properties>\n$1`);
        }
      }
    }

    // 3. Remove invalid XML child elements generated by AI (hallucinations)
    // We only remove these if they are standalone tags, not attributes.
    const invalidTags = ['translatable', 'minimumValue', 'maximumValue', 'defaultValue', 'isList', 'required', 'isDefault', 'onChange', 'returnType'];
    // Specifically for returnType, we only remove it if it's NOT inside action or expression
    invalidTags.forEach(tag => {
      if (tag === 'returnType') {
         // returnType is valid inside expression/action, so we are more careful.
         // Actually, Mendix 10 uses <returnType type="Void" /> etc.
         // Let's just remove 'translatable' and friends which are the main culprits.
         return;
      }
      finalXml = finalXml.replace(new RegExp(`<${tag}>[\\s\\S]*?<\\/${tag}>`, 'gi'), '');
      finalXml = finalXml.replace(new RegExp(`<${tag}\\s*\\/>`, 'gi'), '');
    });
